...
class VerifyAwardsPhase13 extends BaseCommand
{
    protected $group       = 'Verification';
    protected $name        = 'verify:awards-phase-13';
    protected $description = 'Validates Phase 13 Outstanding Athlete of the Year - Male Full Source-Fidelity Remediation';

    public function run(array $params)
    {
        CLI::write("========================================================================", 'cyan');
        CLI::write("AchieveNest — Phase 13: Athlete Male Award Full Stack Verification", 'white');
        CLI::write("========================================================================", 'cyan');
        ...
        $award = $db->table('award_definitions')
            ...
            ->get()->getRowArray();
        $runTest('ATHLETE-M-IDENT-001', 'Exact Name "Outstanding Athlete of the Year - Male"', $award['name'] === 'Outstanding Athlete of the Year - Male' && $award['code'] === 'ATHLETE_OF_THE_YEAR_MALE');
        $runTest('ATHLETE-M-IDENT-002', 'Authority OFFICIAL, Status active, Source Fidelity VERIFIED', $award['authority_status'] === 'OFFICIAL' && $award['status'] === 'active' && $award['source_fidelity_status'] === 'VERIFIED');
        $runTest('ATHLETE-M-IDENT-003', 'Annual cycle (graduating_only = 0) & male restriction', (int) $award['graduating_only'] === 0 && $award['gender_restriction'] === 'male');
        $runTest('ATHLETE-M-IDENT-004', 'Candidate threshold is exactly 80.00%', (float) $award['candidate_threshold_percent'] === 80.00);
        ...
        $criteria = $db->table('award_criteria')
            ...
            ->get()->getResultArray();
        $runTest('ATHLETE-M-OFFIC-001', 'Exact 5 Official Criteria present in Athlete Male Award', count($criteria) === 5);
        $officialTotal = array_sum(array_column($criteria, 'weight'));
        $runTest('ATHLETE-M-OFFIC-002', 'Official Rubric Total sums to exactly 100.00 points', (float) $officialTotal === 100.00);
        ...
        $runTest('ATHLETE-M-COMP-001', 'Computable Criteria max points sum to exactly 55.00 points', (float) $computableTotal === 55.00);

        $nonComputable = array_filter($criteria, fn($c) => (int) $c['is_portfolio_computable'] === 0);
        $runTest('ATHLETE-M-COMP-002', 'Academic (15) and Interview (10) isolated as non-computable', count($nonComputable) === 2);

        // 4. Component Rules Engine Simulation
        $ruleA1 = $db->table('award_scoring_rules')->where('code', 'RULE_ATHLETE_M_INDIV')->get()->getRowArray();
        $configA1 = json_decode($ruleA1['rule_config'], true);
        $indivPts = $configA1['presence_points'] ?? 0;
        $runTest('ATHLETE-M-A1-001', 'Component A1 configured as individual presence (max 10.00)', $ruleA1['rule_type'] === 'sum_capped' && (float) $indivPts === 10.0 && (float) $ruleA1['max_points'] === 10.00);

        $ruleA2 = $db->table('award_scoring_rules')->where('code', 'RULE_ATHLETE_M_TEAM')->get()->getRowArray();
        $configA2 = json_decode($ruleA2['rule_config'], true);
        $teamPts = $configA2['presence_points'] ?? 0;
        $runTest('ATHLETE-M-A2-001', 'Component A2 configured as team presence (max 10.00)', $ruleA2['rule_type'] === 'sum_capped' && (float) $teamPts === 10.0 && (float) $ruleA2['max_points'] === 10.00);

        $ruleB = $db->table('award_scoring_rules')->where('code', 'RULE_ATHLETE_M_PARTICIPATION')->get()->getRowArray();
        $configB = json_decode($ruleB['rule_config'], true);
        $prisaaNat = $configB['event_level_points']['prisaa_national'] ?? 0;
        $runTest('ATHLETE-M-B-001', 'Component B configured as participation matrix (PRISAA Nat=7, max 20.00)', $ruleB['rule_type'] === 'sum_capped' && (float) $prisaaNat === 7.0 && (float) $ruleB['max_points'] === 20.00);

        $ruleC = $db->table('award_scoring_rules')->where('code', 'RULE_ATHLETE_M_AWARDS')->get()->getRowArray();
        $configC = json_decode($ruleC['rule_config'], true);
        $natGold = $configC['medal_matrix']['prisaa_national']['gold'] ?? 0;
        $runTest('ATHLETE-M-C-001', 'Component C configured as medal matrix (Nat Gold=7, max 15.00)', $ruleC['rule_type'] === 'sum_capped' && (float) $natGold === 7.0 && (float) $ruleC['max_points'] === 15.00);
        ...
        $mappings = $db->table('award_evidence_mapping_rules r')
            ...
            ->get()->getResultArray();
        $runTest('ATHLETE-M-MAP-001', 'Exact 4 Evidence Mapping Rules active for Athlete Male Award', count($mappings) === 4);
        ...
        $runTest('ATHLETE-M-SUMM-001', 'Evaluation Summary snapshot produced with complete totals', isset($summary['totals']['portfolio_potential_score']));
        $runTest('ATHLETE-M-SUMM-002', 'Evaluation Summary exposes 55.00 computable max & 100.00 official max', (float) $summary['totals']['max_computable_score'] === 55.00 && (float) $summary['totals']['official_rubric_total'] === 100.00);

        // 7. Cumulative Invariant: 13 Verified / 15 Authoritative Baseline
        ...
        // 8. Cumulative Regression: Awards 01, 02, 03, 04, 05, 06, 07, 08, 09, 10, 11, 12
        ...
        $runTest('SPORTS-M-REGR-001', 'Sports Male preserved: 100 official / 55 computable / Graduating / male / VERIFIED (distinct from Annual Athlete Male)', $sportsM['source_fidelity_status'] === 'VERIFIED' && count($sportsMCriteria) === 5 && (float) $sportsMComputable === 55.00 && $sportsM['gender_restriction'] === 'male' && (int) $sportsM['graduating_only'] === 1);
        ...
        CLI::write("========================================================================", 'cyan');
        CLI::write("Phase 13 Verification Summary: {$passed} Passed, {$failed} Failed", $failed === 0 ? 'green' : 'red');
        CLI::write("========================================================================", 'cyan');
