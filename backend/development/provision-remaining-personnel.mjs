...
const { Pool } = pg
const workbookPath = 'C:/Users/Admin/Downloads/AchieveNest_HR_Personnel_Demo_Roster_Updated.xlsx'
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')
const credentialPath = path.join(root, 'writable', 'demo-credentials', 'achievenest-demo-credentials.txt')
const targetIds = new Set(['9000000002', '9000000003', '9000000004', '9000000005'])
...
if (rosterRows.length !== targetIds.size) throw new Error('The workbook did not contain exactly the four requested personnel rows.')

for (const row of rosterRows) {
  const institutionalId = String(row.institutional_id)
  const email = String(row.institutional_email).trim()
  const result = { institutionalId, email, authUuid: '', authCreation: 'failure', profileCreation: 'failure', roleAssignment: 'failure', lifecycleCreation: 'failure', signIn: 'failure', httpStatus: '', accountType: '', status: '', roleKeys: '', department: '', designation: '', failure: '' }
  ...
  try {
    ...
    try {
      ...
      if (college.rowCount !== 1 || department.rowCount !== 1 || role.rowCount !== 1) throw new Error('College, department, or personnel role did not resolve exactly.')
      if (department.rows[0].college_id !== college.rows[0].id) throw new Error('Department does not belong to college.')

      const firstName = String(row.first_name).trim()
      const middleName = row.middle_name ? String(row.middle_name).trim() : null
      ...
      await database.query(`INSERT INTO public.profiles
        ...
        VALUES ($1, $2, $3, $4, $5, $6, NULL, $7, 'personnel', $8, NULL, NULL, $9, $10, 'roster_xlsx', false, now(), now())`,
      [result.authUuid, institutionalId, email, firstName, middleName, lastName, fullName, department.rows[0].id, String(row.designation).trim(), String(row.account_status).trim().toLowerCase()])
      ...
      await database.query(`INSERT INTO public.account_lifecycle_events (profile_id, event_type, reason)
        VALUES ($1, 'provisioned', 'Created from validated HR personnel XLSX roster'),
               ($1, 'activated', 'Activated from validated HR personnel XLSX roster')`, [result.authUuid])
    ...
    result.roleKeys = (body.data?.user?.roles ?? []).map((item) => item.role_key).join(',')
    result.department = body.data?.user?.department_id ?? ''
    result.designation = body.data?.user?.designation ?? ''
  } catch (error) {
...
for (const result of results) {
  ...
  console.log(`returned role keys: ${result.roleKeys}`)
  console.log(`returned department: ${result.department}`)
  console.log(`returned designation: ${result.designation}`)
}
...
console.log(`failed Personnel rows: ${results.filter((result) => result.failure).map((result) => `${result.institutionalId} (${result.failure})`).join(', ') || 'none'}`)
console.log(`specialized roles assigned: ${roleCount > profilesCount ? 'yes' : 'no'}`)
