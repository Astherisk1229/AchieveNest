...
/**
 * Class EvaluationInstrumentRegistry
 *
 * Authoritative Evaluation Instrument Freeze & Canonical Scoring Configuration Baseline.
 * Plan F — Phase F0.
 *
 * Freezes exact scale definitions, criteria, selectable values, point schedules, caps,
 * formulas, required fields, and evidence rules for:
 * 1. ADMINISTRATORS_RANKING_SCALE (Overall 160, Passing 120)
 * 2. NON_TEACHING_PERSONNEL_RANKING_SCALE (Overall 150, Passing 75)
 */
class EvaluationInstrumentRegistry
{
    public const RULE_VERSION = 'NDMU-PERSONNEL-RATING-V2';
    public const SOURCE_DOC_ADMIN = 'NDMU-DOC-EVAL-ADMIN-2026-V1';
    public const SOURCE_DOC_NON_TEACHING = 'NDMU-DOC-EVAL-NON-TEACHING-2026-V1';
    ...
    /**
     * Canonical Instruments Dictionary
     */
    protected static ?array $instruments = null;

    /**
     * Returns full canonical instruments array.
     */
    public static function getInstruments(): array
    {
        if (self::$instruments === null) {
            self::$instruments = self::buildInstruments();
        }
        return self::$instruments;
    }

    /**
     * Returns an instrument by scale code.
     */
    public static function getInstrument(string $scaleCode): ?array
    {
        $all = self::getInstruments();
        return $all[$scaleCode] ?? null;
    }

    /**
     * Resolves canonical scale code for a personnel classification pair.
     */
    public static function resolveScaleCode(string $personnelGroup, string $organizationalSide): string
    {
        $group = strtolower(trim($personnelGroup));
        $side = strtolower(trim($organizationalSide));

        if ($group === 'faculty' && $side === 'academic') {
            return self::SCALE_ADMINISTRATORS;
        }
        ...
        if ($group === 'non_teaching_faculty' && $side === 'non_academic') {
            return self::SCALE_NON_TEACHING;
        }

        throw new RuntimeException("Invalid personnel classification combination: [{$group} + {$side}]. No authoritative evaluation scale assigned.", 422);
    }

    /**
     * Builds the complete immutable frozen data structure.
     */
    protected static function buildInstruments(): array
    {
        return [
            self::SCALE_ADMINISTRATORS => [
                'scale_code' => self::SCALE_ADMINISTRATORS,
                'title' => 'Rating Sheet for Administrators & Academic Personnel',
                'rule_version' => self::RULE_VERSION,
                'source_document_id' => self::SOURCE_DOC_ADMIN,
            ...
            self::SCALE_NON_TEACHING => [
                'scale_code' => self::SCALE_NON_TEACHING,
                'title' => 'Non-Teaching Personnel Rating Sheet for Ranking (Appendix N)',
                'rule_version' => self::RULE_VERSION,
                'source_document_id' => self::SOURCE_DOC_NON_TEACHING,
