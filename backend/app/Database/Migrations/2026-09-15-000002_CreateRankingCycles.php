...
class CreateRankingCycles extends Migration
{
    public function up()
    {
        $this->db->query(<<<'SQL'
...
SQL);
    ...
    {
        if ($this->db->fieldExists('ranking_cycle_id', 'personnel_evaluation_periods')) {
            $this->db->query('ALTER TABLE personnel_evaluation_periods DROP FOREIGN KEY fk_personnel_period_cycle');
            $this->forge->dropColumn('personnel_evaluation_periods', 'ranking_cycle_id');
        }
        $this->forge->dropTable('ranking_cycles', true);
    }

    private function uuid(): string
    {
        return sprintf('%04x%04x-%04x-4%03x-%04x-%04x%04x%04x', random_int(0,65535), random_int(0,65535), random_int(0,65535), random_int(0,4095), random_int(32768,49151), random_int(0,65535), random_int(0,65535), random_int(0,65535));
    }
}
