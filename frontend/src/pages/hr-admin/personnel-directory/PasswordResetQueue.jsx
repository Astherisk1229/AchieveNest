  ...
  return (
    <div className="space-y-4">
      ...
      {filteredResets.length === 0 ? (
        <div className="p-8 text-center text-sm text-slate-400 font-normal rounded-2xl bg-white dark:bg-[#131e2e] border border-slate-200/80 dark:border-slate-800">
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-0.5">
                      <p className="text-sm font-semibold text-slate-900 dark:text-white">{req.user_name || req.user_email}</p>
                      <p className="text-xs font-normal text-slate-500">{req.user_email}</p>
                    </div>
                  </div>
                ...
                {!isApproved && (
                  <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200/80 space-y-2">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Generated Temp Password</p>
                      <button
                        type="button"
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <code className="font-mono text-xs font-semibold text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-800 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700 flex-1">
                        {tempPass}
                      </code>
                      <button
                        type="button"
                        onClick={() => handleCopy(req.id, tempPass)}
                        className="px-2.5 py-1 rounded-lg bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold text-xs hover:bg-slate-300 flex items-center gap-1 transition cursor-pointer"
                      >
