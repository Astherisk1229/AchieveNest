...
class VerifyAwardsPhase6 extends BaseCommand
{
    protected $group       = 'Verification';
    protected $name        = 'verify:awards-phase-6';
    protected $description = 'Validates Phase 6 Outstanding Performance in Sports - Male Full Source-Fidelity Remediation';

    public function run(array $params)
    {
        CLI::write("========================================================================", 'cyan');
        CLI::write("AchieveNest — Phase 6: Sports Male Award Full Stack Verification", 'white');
        CLI::write("========================================================================", 'cyan');
        ...
        $award = $db->table('award_definitions')
            ...
            ->get()->getRowArray();
        $runTest('SPORTS-M-IDENT-001', 'Exact Name "Outstanding Performance in Sports - Male"', $award['name'] === 'Outstanding Performance in Sports - Male' && $award['code'] === 'SPORTS_AWARD_MALE');
        $runTest('SPORTS-M-IDENT-002', 'Authority OFFICIAL, Status active, Source Fidelity VERIFIED', $award['authority_status'] === 'OFFICIAL' && $award['status'] === 'active' && $award['source_fidelity_status'] === 'VERIFIED');
        $runTest('SPORTS-M-IDENT-003', 'Graduating only & male restriction are active', (int) $award['graduating_only'] === 1 && $award['gender_restriction'] === 'male');
        $runTest('SPORTS-M-IDENT-004', 'Candidate threshold is exactly 80.00%', (float) $award['candidate_threshold_percent'] === 80.00);
        ...
        $criteria = $db->table('award_criteria')
            ...
            ->get()->getResultArray();
        $runTest('SPORTS-M-OFFIC-001', 'Exact 5 Official Criteria present in Sports Male Award', count($criteria) === 5);
        $officialTotal = array_sum(array_column($criteria, 'weight'));
        $runTest('SPORTS-M-OFFIC-002', 'Official Rubric Total sums to exactly 100.00 points', (float) $officialTotal === 100.00);
        ...
        $runTest('SPORTS-M-COMP-001', 'Computable Criteria max points sum to exactly 55.00 points', (float) $computableTotal === 55.00);

        $nonComputable = array_filter($criteria, fn($c) => (int) $c['is_portfolio_computable'] === 0);
        $runTest('SPORTS-M-COMP-002', 'Academic (15) and Interview (10) isolated as non-computable', count($nonComputable) === 2);

        // 4. Component Rules Engine Simulation
        $ruleA1 = $db->table('award_scoring_rules')->where('code', 'RULE_SPORTS_M_INDIV')->get()->getRowArray();
        $runTest('SPORTS-M-A1-001', 'Component A1 rule configured as individual presence (max 10.00)', $ruleA1['rule_type'] === 'sum_capped' && (float) $ruleA1['max_points'] === 10.00);

        $ruleA2 = $db->table('award_scoring_rules')->where('code', 'RULE_SPORTS_M_TEAM')->get()->getRowArray();
        $runTest('SPORTS-M-A2-001', 'Component A2 rule configured as team presence (max 10.00)', $ruleA2['rule_type'] === 'sum_capped' && (float) $ruleA2['max_points'] === 10.00);

        $ruleB = $db->table('award_scoring_rules')->where('code', 'RULE_SPORTS_M_PARTICIPATION')->get()->getRowArray();
        $configB = json_decode($ruleB['rule_config'], true);
        $prisaaNat = $configB['event_level_points']['prisaa_national'] ?? 0;
        $runTest('SPORTS-M-B-001', 'Component B rule configured as participation matrix (max 20.00)', $ruleB['rule_type'] === 'sum_capped' && (float) $prisaaNat === 7.0 && (float) $ruleB['max_points'] === 20.00);

        $ruleC = $db->table('award_scoring_rules')->where('code', 'RULE_SPORTS_M_AWARDS')->get()->getRowArray();
        $configC = json_decode($ruleC['rule_config'], true);
        $prisaaNatGold = $configC['medal_matrix']['prisaa_national']['gold'] ?? 0;
        $runTest('SPORTS-M-C-001', 'Component C rule configured as medal matrix (max 15.00)', $ruleC['rule_type'] === 'sum_capped' && (float) $prisaaNatGold === 7.0 && (float) $ruleC['max_points'] === 15.00);
        ...
        $mappings = $db->table('award_evidence_mapping_rules r')
            ...
            ->get()->getResultArray();
        $runTest('SPORTS-M-MAP-001', 'Exact 4 Evidence Mapping Rules active for Sports Male', count($mappings) === 4);
        ...
        $runTest('SPORTS-M-SUMM-001', 'Evaluation Summary snapshot produced with complete totals', isset($summary['totals']['portfolio_potential_score']));
        $runTest('SPORTS-M-SUMM-002', 'Evaluation Summary exposes 55.00 computable max & 100.00 official max', (float) $summary['totals']['max_computable_score'] === 55.00 && (float) $summary['totals']['official_rubric_total'] === 100.00);

        // 7. Cumulative Invariant: 6 Verified / 15 Authoritative Baseline
        ...
        // 8. Cumulative Regression: Awards 01, 02, 03, 04, 05
        ...
        CLI::write("========================================================================", 'cyan');
        CLI::write("Phase 6 Verification Summary: {$passed} Passed, {$failed} Failed", $failed === 0 ? 'green' : 'red');
        CLI::write("========================================================================", 'cyan');
