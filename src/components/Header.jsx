...
export default function Header({ currentUser, onToggleSidebar, onRoleChange }) {
  ...
  const activeRoleContext = user?.active_role_context || user?.user_type || 'personnel'
  ...
  // All potential personnel role options with their icons
  ...
  // Filter out the currently active role context so ONLY alternate available roles show under "Switch To"
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="text-left hidden sm:block">
              <p className="text-xs font-bold text-slate-900 leading-tight">{user?.full_name || 'Dr. Maria Santos'}</p>
              <div className="flex items-center gap-1 mt-0.5">
                <span className="text-[11px] font-medium text-slate-500 capitalize flex items-center gap-1">
                  ...
                  {user?.user_type === 'personnel' ? 'Personnel' : user?.user_type?.replace('_', ' ')}
                </span>
          ...
          {isProfileOpen && (
            <>
              ...
              <div className="absolute right-0 mt-2 w-72 rounded-3xl bg-white text-slate-900 shadow-2xl border border-slate-200/90 p-3.5 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="flex items-center gap-3 p-3 bg-slate-50/80 rounded-2xl border border-slate-100 mb-2">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-extrabold text-slate-900 truncate">{user?.full_name || 'Dr. Maria Santos'}</p>
                    <p className="text-[11px] text-slate-500 font-medium capitalize flex items-center gap-1 mt-0.5">
                      <User className="w-3 h-3 text-[#2d8a4e]" /> Personnel
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-1 py-1">
                  ...
                  <div className="pt-1">
                    ...
                    {/* Expandable Role Options List (Excludes Currently Active Role) */}
                    {isSwitchToOpen && (
