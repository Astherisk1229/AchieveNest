...
| --- | --- |
| Conversion | Hex/rgb/hsl to oklch |
| Palettes | Generate scales, multi-hue, dark mode |
| Contrast | APCA/WCAG checks, reporting failures, fixing on request |
| Gamut & Tailwind | P3 fallbacks, `@theme` scales, gamut clamping |
| Usage | Semantic tokens, one meaning per color, primary-action emphasis, appearance variants |
...
| Failing contrast | Report the pair, its measured Lc and the threshold it misses; change colors only when asked |
| High chroma without gamut check | Clamp to max chroma for the L/H in sRGB |
| Same hue means two different things | One color, one meaning; give the second use a neutral |
| Semantic token used outside its role | Add a token for the missing role; never borrow by value |
