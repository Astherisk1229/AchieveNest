...
class VerifyAwardsPhase14 extends BaseCommand
{
    protected $group       = 'Verification';
    protected $name        = 'verify:awards-phase-14';
    protected $description = 'Validates Phase 14 Outstanding Performer of the Year - Female Proposed Model Source-Fidelity Remediation';

    public function run(array $params)
    {
        CLI::write("========================================================================", 'cyan');
        CLI::write("AchieveNest — Phase 14: Performer Female Proposed Verification", 'white');
        CLI::write("========================================================================", 'cyan');
        ...
        $award = $db->table('award_definitions')
            ...
            ->get()->getRowArray();
        $runTest('PERFORMER-F-IDENT-001', 'Exact Name "Outstanding Performer of the Year - Female"', $award['name'] === 'Outstanding Performer of the Year - Female' && $award['code'] === 'PERFORMER_OF_THE_YEAR_FEMALE');
        $runTest('PERFORMER-F-IDENT-002', 'Authority PROPOSED, Status active, Source Fidelity VERIFIED', $award['authority_status'] === 'PROPOSED' && $award['status'] === 'active' && $award['source_fidelity_status'] === 'VERIFIED');
        $runTest('PERFORMER-F-IDENT-003', 'Annual cycle (graduating_only = 0) & female restriction', (int) $award['graduating_only'] === 0 && $award['gender_restriction'] === 'female');
        $runTest('PERFORMER-F-IDENT-004', 'Candidate threshold is exactly 80.00%', (float) $award['candidate_threshold_percent'] === 80.00);
        ...
        $criteria = $db->table('award_criteria')
            ...
            ->get()->getResultArray();
        $runTest('PERFORMER-F-PROP-001', 'Exact 3 Proposed Criteria present in Performer Female Award', count($criteria) === 3);
        $computableTotal = array_sum(array_column($criteria, 'max_points'));
        $runTest('PERFORMER-F-PROP-002', 'Proposed Computable max points sum to exactly 55.00 points', (float) $computableTotal === 55.00);

        $officialCriteria = array_filter($criteria, fn($c) => $c['authority_status'] === 'OFFICIAL');
        $runTest('PERFORMER-F-PROP-003', 'No fake official 100-point rubric synthesized', count($officialCriteria) === 0);

        // 3. Component Rules Engine Simulation
        $ruleA1 = $db->table('award_scoring_rules')->where('code', 'RULE_PERFORMER_F_INDIV')->get()->getRowArray();
        $configA1 = json_decode($ruleA1['rule_config'], true);
        $indivPts = $configA1['presence_points'] ?? 0;
        $runTest('PERFORMER-F-A1-001', 'Component A1 configured as individual presence (max 10.00)', $ruleA1['rule_type'] === 'sum_capped' && (float) $indivPts === 10.0 && (float) $ruleA1['max_points'] === 10.00);

        $ruleA2 = $db->table('award_scoring_rules')->where('code', 'RULE_PERFORMER_F_GROUP')->get()->getRowArray();
        $configA2 = json_decode($ruleA2['rule_config'], true);
        $groupPts = $configA2['presence_points'] ?? 0;
        $runTest('PERFORMER-F-A2-001', 'Component A2 configured as group presence (max 10.00)', $ruleA2['rule_type'] === 'sum_capped' && (float) $groupPts === 10.0 && (float) $ruleA2['max_points'] === 10.00);

        $ruleB = $db->table('award_scoring_rules')->where('code', 'RULE_PERFORMER_F_PARTICIPATION')->get()->getRowArray();
        $configB = json_decode($ruleB['rule_config'], true);
        $prisaaNat = $configB['event_level_points']['prisaa_national'] ?? 0;
        $runTest('PERFORMER-F-B-001', 'Component B configured as participation matrix (PRISAA Nat=7, max 20.00)', $ruleB['rule_type'] === 'sum_capped' && (float) $prisaaNat === 7.0 && (float) $ruleB['max_points'] === 20.00);

        $ruleC = $db->table('award_scoring_rules')->where('code', 'RULE_PERFORMER_F_AWARDS')->get()->getRowArray();
        ...
        $runTest('PERFORMER-F-C-001', 'Component C configured as placement matrix (Nat Gold=7, NDEA Silver=3, max 15.00)', $ruleC['rule_type'] === 'sum_capped' && (float) $natGold === 7.0 && (float) $ndeaSilver === 3.0 && (float) $ruleC['max_points'] === 15.00);
        ...
        $mappings = $db->table('award_evidence_mapping_rules r')
            ...
            ->get()->getResultArray();
        $runTest('PERFORMER-F-MAP-001', 'Exact 4 Evidence Mapping Rules active for Performer Female Award', count($mappings) === 4);
        ...
        $runTest('PERFORMER-F-SUMM-001', 'Evaluation Summary snapshot produced with complete totals', isset($summary['totals']['portfolio_potential_score']));
        $runTest('PERFORMER-F-SUMM-002', 'Evaluation Summary exposes 55.00 computable max & PROPOSED authority', (float) $summary['totals']['max_computable_score'] === 55.00 && $summary['award']['authority_status'] === 'PROPOSED');

        // 6. Cumulative Invariant: 14 Verified / 15 Authoritative Baseline
        ...
        // 7. Cumulative Regression: Awards 01, 02, 03, 04, 05, 06, 07, 08, 09, 10, 11, 12, 13
        ...
        $runTest('SOCIO-F-REGR-001', 'Socio-Cultural Female preserved: PROPOSED / 55 computable / Graduating / female / VERIFIED (distinct from Annual Performer Female)', $socioF['source_fidelity_status'] === 'VERIFIED' && count($socioFCriteria) === 3 && (float) $socioFComputable === 55.00 && $socioF['authority_status'] === 'PROPOSED' && (int) $socioF['graduating_only'] === 1);
        ...
        CLI::write("========================================================================", 'cyan');
        CLI::write("Phase 14 Verification Summary: {$passed} Passed, {$failed} Failed", $failed === 0 ? 'green' : 'red');
        CLI::write("========================================================================", 'cyan');
