...
export default function OSADDepartmentsProgramsPage({
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-200 font-sans">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <ArrowLeft className="w-4 h-4 text-[#064e2b] dark:text-emerald-400" />
          ...
          <div className="flex items-center gap-1 text-xs font-bold text-slate-400">
            ...
            <span className="text-[#064e2b] dark:text-emerald-400 font-black">{dept.code}</span>
          </div>
        </div>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-semibold transition flex items-center gap-1.5 shadow-2xs ${
                  !canCreateDegreeProgram
                    ...
                    : 'bg-[#EFF7F0] hover:bg-[#143326] text-white border border-transparent cursor-pointer'
                }`}
              >
        ...
        <div className="bg-white dark:bg-[#131e2e] rounded-2xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-2xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
            <h3 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
              <GraduationCap className="w-4 h-4 text-[#064e2b] dark:text-emerald-400" />
          ...
          <div className="space-y-3">
            {(dept.programs || []).map((prog) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <span className="w-2.5 h-2.5 rounded-full bg-[#EFF7F0] shrink-0" />
                  <div>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <span className="text-[9px] font-extrabold text-[#16834a] dark:text-emerald-400 uppercase block">Program Coordinator</span>
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-xl bg-white dark:bg-slate-800 hover:bg-emerald-50 text-[#064e2b] dark:text-[#245F42] font-extrabold text-xs border border-slate-200 dark:border-slate-700 transition cursor-pointer flex items-center gap-1.5 shadow-2xs shrink-0"
                  >
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200 font-sans">
      ...
      {/* Page Header */}
      <div className="bg-white dark:bg-[#1D2A23] rounded-2xl p-5 sm:p-6 border border-[#DCE6DF] dark:border-[#374B3F] shadow-xs flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-6">
        <div className="flex items-center gap-3.5 min-w-0">
          <div className="w-10 h-10 rounded-xl bg-[#E7F5EA] dark:bg-emerald-950/50 text-[#16834A] dark:text-emerald-400 border border-[#B7DDC4] dark:border-emerald-800/50 flex items-center justify-center shrink-0">
            <Building2 className="w-5 h-5" />
          </div>
          <div className="min-w-0">
            <h1 className="text-xl font-extrabold text-[#102A43] dark:text-[#E6EFE9] tracking-tight">
              Academic Structure
            </h1>
            <p className="text-xs text-[#4F6475] dark:text-[#B1C0B6] font-medium mt-0.5">
        ...
        <div 
          className="create-action-group flex flex-col sm:flex-row sm:items-center sm:flex-nowrap gap-2.5 sm:gap-3 w-full lg:w-auto shrink-0"
        ...
        >
          ...
          <button
            ...
            className="create-action-button create-action-button-primary shadow-xs text-xs whitespace-nowrap shrink-0"
          >
          ...
          <div className="relative group shrink-0" title={departmentTooltip || undefined}>
            <button
              type="button"
              ...
              className={`create-action-button text-xs whitespace-nowrap shrink-0 ${
                !canCreateDepartment
                  ? 'bg-[#E6ECE8] dark:bg-slate-900/60 text-[#87958C] dark:text-slate-600 border border-[#D4DED7] dark:border-slate-800 cursor-not-allowed'
                  : 'create-action-button-secondary shadow-xs'
              }`}
            >
              <Plus className="w-4 h-4 stroke-[2.5]" aria-hidden="true" />
          ...
          <div className="relative group shrink-0" title={degreeProgramTooltip || undefined}>
            <button
              type="button"
              disabled={!canCreateDegreeProgram}
        ...
        {departments.map((dept) => {
          ...
          return (
            <div 
              ...
              className="bg-white dark:bg-[#131e2e] rounded-2xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-2xs space-y-4 flex flex-col justify-between hover:border-[#69A97C] dark:hover:border-emerald-600 cursor-pointer transition duration-150 group"
            >
              <div className="space-y-3">
                ...
                <div className="border-b border-slate-100 dark:border-slate-800 pb-3 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black text-[#16834a] dark:text-emerald-400 tracking-wider uppercase group-hover:underline">
                      {dept.code} DEPARTMENT
                    </span>
                  ...
                  <h3 className="font-extrabold text-sm text-slate-900 dark:text-white leading-snug group-hover:text-[#064e2b] dark:group-hover:text-emerald-400 transition">
                    {dept.name}
                  </h3>
                </div>
                ...
                <div className="space-y-2">
                  <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                    <GraduationCap className="w-3.5 h-3.5 text-[#16834a] dark:text-emerald-400" />
                    <span>Degree Programs ({(dept.programs || []).length})</span>
                  </p>
                    ...
                    {(dept.programs || []).map((prog) => (
                      <li key={prog} className="pt-1.5 first:pt-0 flex items-center justify-between text-xs font-semibold">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="w-1.5 h-1.5 rounded-full bg-[#16834a] shrink-0" />
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 space-y-2" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <span className="text-[9px] font-extrabold text-[#16834a] dark:text-emerald-400 uppercase block">
                      Program Coordinator
                    </span>
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-white dark:bg-slate-800 hover:bg-emerald-50 text-[#064e2b] dark:text-emerald-400 font-extrabold text-xs border border-slate-200 dark:border-slate-700 transition cursor-pointer flex items-center gap-1 shrink-0 shadow-2xs"
                  >
                    <ShieldCheck className="w-3.5 h-3.5" />
                    <span>Assign</span>
                  </button>
