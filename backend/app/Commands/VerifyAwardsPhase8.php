...
class VerifyAwardsPhase8 extends BaseCommand
{
    protected $group       = 'Verification';
    protected $name        = 'verify:awards-phase-8';
    protected $description = 'Validates Phase 8 Outstanding Performance in Socio-Cultural - Male Proposed Model Remediation';

    public function run(array $params)
    {
        CLI::write("========================================================================", 'cyan');
        CLI::write("AchieveNest — Phase 8: Socio-Cultural Male Award Full Stack Verification", 'white');
        CLI::write("========================================================================", 'cyan');
        ...
        $award = $db->table('award_definitions')
            ...
            ->get()->getRowArray();
        $runTest('SOCIO-M-IDENT-001', 'Exact Name "Outstanding Performance in Socio-Cultural - Male"', $award['name'] === 'Outstanding Performance in Socio-Cultural - Male' && $award['code'] === 'SOCIO_CULTURAL_AWARD_MALE');
        $runTest('SOCIO-M-IDENT-002', 'Authority PROPOSED, Status active, Source Fidelity VERIFIED', $award['authority_status'] === 'PROPOSED' && $award['status'] === 'active' && $award['source_fidelity_status'] === 'VERIFIED');
        $runTest('SOCIO-M-IDENT-003', 'Graduating only & male restriction are active', (int) $award['graduating_only'] === 1 && $award['gender_restriction'] === 'male');
        $runTest('SOCIO-M-IDENT-004', 'Candidate threshold is exactly 80.00%', (float) $award['candidate_threshold_percent'] === 80.00);
        ...
        $criteria = $db->table('award_criteria')
            ...
            ->get()->getResultArray();
        $runTest('SOCIO-M-OFFIC-001', 'Exact 3 Proposed Criteria present in Socio-Cultural Male Award', count($criteria) === 3);
        $totalWeight = array_sum(array_column($criteria, 'weight'));
        $runTest('SOCIO-M-OFFIC-002', 'Proposed Model Total sums to exactly 55.00 points', (float) $totalWeight === 55.00);
        ...
        $runTest('SOCIO-M-COMP-001', 'Computable Criteria max points sum to exactly 55.00 points', (float) $computableTotal === 55.00);

        $nonComputable = array_filter($criteria, fn($c) => (int) $c['is_portfolio_computable'] === 0);
        $runTest('SOCIO-M-COMP-002', 'Zero non-computable criteria (no fake 100 pt official rubric)', count($nonComputable) === 0);

        // 4. Component Rules Engine Simulation
        $ruleA1 = $db->table('award_scoring_rules')->where('code', 'RULE_SOCIO_M_INDIV')->get()->getRowArray();
        $runTest('SOCIO-M-A1-001', 'Component A1 rule configured as individual presence (max 10.00)', $ruleA1['rule_type'] === 'sum_capped' && (float) $ruleA1['max_points'] === 10.00);

        $ruleA2 = $db->table('award_scoring_rules')->where('code', 'RULE_SOCIO_M_GROUP')->get()->getRowArray();
        $runTest('SOCIO-M-A2-001', 'Component A2 rule configured as group presence (max 10.00)', $ruleA2['rule_type'] === 'sum_capped' && (float) $ruleA2['max_points'] === 10.00);

        $ruleB = $db->table('award_scoring_rules')->where('code', 'RULE_SOCIO_M_PARTICIPATION')->get()->getRowArray();
        $configB = json_decode($ruleB['rule_config'], true);
        $prisaaNat = $configB['event_level_points']['prisaa_national'] ?? 0;
        $runTest('SOCIO-M-B-001', 'Component B rule configured as participation matrix (max 20.00)', $ruleB['rule_type'] === 'sum_capped' && (float) $prisaaNat === 7.0 && (float) $ruleB['max_points'] === 20.00);

        $ruleC = $db->table('award_scoring_rules')->where('code', 'RULE_SOCIO_M_AWARDS')->get()->getRowArray();
        $configC = json_decode($ruleC['rule_config'], true);
        $ndeaSilver = $configC['medal_matrix']['ndea']['silver'] ?? 0;
        $runTest('SOCIO-M-C-001', 'Component C rule configured with NDEA Silver = 3 (max 15.00)', $ruleC['rule_type'] === 'sum_capped' && (float) $ndeaSilver === 3.0 && (float) $ruleC['max_points'] === 15.00);
        ...
        $mappings = $db->table('award_evidence_mapping_rules r')
            ...
            ->get()->getResultArray();
        $runTest('SOCIO-M-MAP-001', 'Exact 4 Evidence Mapping Rules active for Socio-Cultural Male', count($mappings) === 4);
        ...
        $runTest('SOCIO-M-SUMM-001', 'Evaluation Summary snapshot produced with complete totals', isset($summary['totals']['portfolio_potential_score']));
        $runTest('SOCIO-M-SUMM-002', 'Evaluation Summary exposes 55.00 computable max & PROPOSED authority', (float) $summary['totals']['max_computable_score'] === 55.00 && $award['authority_status'] === 'PROPOSED');

        // 7. Cumulative Invariant: 8 Verified / 15 Authoritative Baseline
        ...
        // 8. Cumulative Regression: Awards 01, 02, 03, 04, 05, 06, 07
        ...
        CLI::write("========================================================================", 'cyan');
        CLI::write("Phase 8 Verification Summary: {$passed} Passed, {$failed} Failed", $failed === 0 ? 'green' : 'red');
        CLI::write("========================================================================", 'cyan');
