...
class VerifyAwardsPhase10 extends BaseCommand
{
    protected $group       = 'Verification';
    protected $name        = 'verify:awards-phase-10';
    protected $description = 'Validates Phase 10 Outstanding Member of the Year Full Source-Fidelity Remediation';

    public function run(array $params)
    {
        CLI::write("========================================================================", 'cyan');
        CLI::write("AchieveNest — Phase 10: Member of the Year Full Stack Verification", 'white');
        CLI::write("========================================================================", 'cyan');
        ...
        $award = $db->table('award_definitions')
            ...
            ->get()->getRowArray();
        $runTest('MEMBER-YR-IDENT-001', 'Exact Name "Outstanding Member of the Year"', $award['name'] === 'Outstanding Member of the Year' && $award['code'] === 'MEMBER_OF_THE_YEAR');
        $runTest('MEMBER-YR-IDENT-002', 'Authority OFFICIAL, Status active, Source Fidelity VERIFIED', $award['authority_status'] === 'OFFICIAL' && $award['status'] === 'active' && $award['source_fidelity_status'] === 'VERIFIED');
        $runTest('MEMBER-YR-IDENT-003', 'Annual cycle (graduating_only = 0) & no gender restriction', (int) $award['graduating_only'] === 0 && $award['gender_restriction'] === null);
        $runTest('MEMBER-YR-IDENT-004', 'Candidate threshold is exactly 80.00%', (float) $award['candidate_threshold_percent'] === 80.00);
        ...
        $criteria = $db->table('award_criteria')
            ...
            ->get()->getResultArray();
        $runTest('MEMBER-YR-OFFIC-001', 'Exact 5 Official Criteria present in Member Award', count($criteria) === 5);
        $officialTotal = array_sum(array_column($criteria, 'weight'));
        $runTest('MEMBER-YR-OFFIC-002', 'Official Rubric Total sums to exactly 100.00 points', (float) $officialTotal === 100.00);

        // 3. Portfolio-Computable Criteria (40 pts)
        ...
        $runTest('MEMBER-YR-COMP-001', 'Computable Criteria max points sum to exactly 40.00 points', (float) $computableTotal === 40.00);

        $nonComputable = array_filter($criteria, fn($c) => (int) $c['is_portfolio_computable'] === 0);
        $runTest('MEMBER-YR-COMP-002', 'Scholastic (15), Character (20), Interview (15) isolated as non-computable', count($nonComputable) === 3);

        // 4. Component Rules Engine Simulation
        $ruleA = $db->table('award_scoring_rules')->where('code', 'RULE_MEMBER_YR_INVOLVEMENT')->get()->getRowArray();
        $configA = json_decode($ruleA['rule_config'], true);
        $activityPts = $configA['type_points']['activity_program'] ?? 0;
        $runTest('MEMBER-YR-A-001', 'Component A configured as involvement sum_capped (activity=2, max 20.00)', $ruleA['rule_type'] === 'sum_capped' && (float) $activityPts === 2.0 && (float) $ruleA['max_points'] === 20.00);

        $ruleB = $db->table('award_scoring_rules')->where('code', 'RULE_MEMBER_YR_CONTRIBUTION')->get()->getRowArray();
        $configB = json_decode($ruleB['rule_config'], true);
        $supportPts = $configB['role_points']['contributor_support'] ?? 0;
        $runTest('MEMBER-YR-B-001', 'Component B configured as contribution responsibility (support=2, max 10.00)', $ruleB['rule_type'] === 'sum_capped' && (float) $supportPts === 2.0 && (float) $ruleB['max_points'] === 10.00);

        $ruleC1 = $db->table('award_scoring_rules')->where('code', 'RULE_MEMBER_YR_LEAD_INVOLVEMENT')->get()->getRowArray();
        $configC1 = json_decode($ruleC1['rule_config'], true);
        $ssgPts = $configC1['level_points']['ssg_collegiate_council'] ?? 0;
        $runTest('MEMBER-YR-C1-001', 'Component C1 configured as leadership involvement (SSG=3, max 5.00)', $ruleC1['rule_type'] === 'sum_capped' && (float) $ssgPts === 3.0 && (float) $ruleC1['max_points'] === 5.00);

        $ruleC2 = $db->table('award_scoring_rules')->where('code', 'RULE_MEMBER_YR_LEAD_AWARDS')->get()->getRowArray();
        $configC2 = json_decode($ruleC2['rule_config'], true);
        $intNatPts = $configC2['type_points']['international_national_award'] ?? 0;
        $hasSeminar = isset($configC2['type_points']['seminar']);
        $runTest('MEMBER-YR-C2-001', 'Component C2 configured as honors/citations (Int/Nat=3, no seminar, max 5.00)', $ruleC2['rule_type'] === 'sum_capped' && (float) $intNatPts === 3.0 && ! $hasSeminar && (float) $ruleC2['max_points'] === 5.00);
        ...
        $mappings = $db->table('award_evidence_mapping_rules r')
            ...
            ->get()->getResultArray();
        $runTest('MEMBER-YR-MAP-001', 'Exact 4 Evidence Mapping Rules active for Member Award', count($mappings) === 4);
        ...
        $runTest('MEMBER-YR-SUMM-001', 'Evaluation Summary snapshot produced with complete totals', isset($summary['totals']['portfolio_potential_score']));
        $runTest('MEMBER-YR-SUMM-002', 'Evaluation Summary exposes 40.00 computable max & 100.00 official max', (float) $summary['totals']['max_computable_score'] === 40.00 && (float) $summary['totals']['official_rubric_total'] === 100.00);

        // 7. Cumulative Invariant: 10 Verified / 15 Authoritative Baseline
        ...
        // 8. Cumulative Regression: Awards 01, 02, 03, 04, 05, 06, 07, 08, 09
        ...
        CLI::write("========================================================================", 'cyan');
        CLI::write("Phase 10 Verification Summary: {$passed} Passed, {$failed} Failed", $failed === 0 ? 'green' : 'red');
        CLI::write("========================================================================", 'cyan');
