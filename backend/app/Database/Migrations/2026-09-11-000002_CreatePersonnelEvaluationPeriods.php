...
SQL);
        ...
        foreach (['personnel_evaluations', 'personnel_evaluation_roots', 'personnel_annual_reviews'] as $table) {
            if ($this->db->tableExists($table) && ! $this->db->fieldExists('evaluation_period_id', $table)) {
                $this->forge->addColumn($table, ['evaluation_period_id' => ['type' => 'VARCHAR', 'constraint' => 36, 'null' => true]]);
                $this->db->query("CREATE INDEX idx_{$table}_period ON {$table} (evaluation_period_id)");
                $this->db->query("ALTER TABLE {$table} ADD CONSTRAINT fk_{$table}_period FOREIGN KEY (evaluation_period_id) REFERENCES personnel_evaluation_periods(id)");
            }
        }
        ...
        if ($this->db->tableExists('personnel_evaluations')) {
            foreach ([
                ...
                if (! $this->db->fieldExists($field, 'personnel_evaluations')) $this->forge->addColumn('personnel_evaluations', [$field => $definition]);
            }
        }
        ...
        if ($this->db->tableExists('personnel_qualification_reviews')) {
            foreach ([
                ...
                if (! $this->db->fieldExists($field, 'personnel_qualification_reviews')) $this->forge->addColumn('personnel_qualification_reviews', [$field => $definition]);
            }
        }
    }
    ...
    public function down()
    {
        foreach (['personnel_annual_reviews', 'personnel_evaluation_roots', 'personnel_evaluations'] as $table) {
            if ($this->db->tableExists($table) && $this->db->fieldExists('evaluation_period_id', $table)) $this->forge->dropColumn($table, 'evaluation_period_id');
        }
