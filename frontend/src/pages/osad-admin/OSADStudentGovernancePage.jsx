...
export default function OSADStudentGovernancePage({
  userSearchTerm,
  setUserSearchTerm,
  selectedCollege,
  setSelectedCollege,
  ...
  const rawUsersList = getUsers('student', userSearchTerm, selectedCollege, selectedSort)

  const usersList = useMemo(() => {
    const list = [...rawUsersList]
    if (sortDirection === 'desc') {
      list.reverse()
    }
    return list
  }, [rawUsersList, sortDirection])
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              Student Governance &amp; Accounts Directory
            </h2>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={() => showToast && showToast('Student Account Creation & CSV Import Modal Opened')}
                ...
                <tr className="bg-slate-50 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-800 text-slate-500 font-extrabold uppercase tracking-wider select-none">
                  ...
                  <th className="p-4">
                    <button
                      ...
                      className="flex items-center gap-1.5 hover:text-[#2d8a4e] transition cursor-pointer group"
                      title="Click to sort by Points & Proofs"
                    >
                      <span>Points &amp; Proofs</span>
                      {selectedSort === 'points' ? (
                        sortDirection === 'asc' ? (
                          <ArrowUp className="w-3.5 h-3.5 text-[#2d8a4e]" />
                        ) : (
                    ...
                    <tr key={user.id || user.student_id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition">
                      ...
                      <td className="p-4 pr-6 text-right relative">
                        <div className="relative inline-block text-left">
                          ...
                          {activeRowMenu === (user.id || user.student_id) && (
                            <div className="absolute right-0 mt-1 w-44 rounded-2xl bg-white dark:bg-[#131e2e] border border-slate-200 dark:border-slate-800 shadow-xl z-30 p-1.5 animate-in fade-in zoom-in-95 duration-100">
                              <button
                                type="button"
                                onClick={() => {
